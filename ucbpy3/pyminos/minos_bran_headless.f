...
c      open(7,file=filnam,status='old',form='formatted',iostat=iret)
c      print *,'output file:'
c      read(5,100) filnam
cc MB added one line below
c      print *,filnam(1:lnblnk(filnam))
c      open(8,file=filnam,form='formatted',iostat=iret)
c      call model(7,8)
c      close(7)
...
c      print *,filnam(1:lnblnk(filnam))
c      ifreq=1
c      if(filnam(1:4).eq.'none') ifreq=0
c      open(3,file=filnam,form='unformatted',iostat=iret)
...
c sfrench start
c     run_minos
c     a front-end for calling minos from cminos
      subroutine run_minos()
      call model(0,0)
c     hard-wire eigenfunction calculation on - otherwise group velocity
c     estimates (poor ones, really) are returned
      ifreq=1
      call wtable(0,0,ifreq)
      end
c sfrench stop
...
c*** makes up table of frequencies ***
     ...
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
      ...
      stepf=1.d0
c sfrench start
c     replaces interactive configuration below ...
...
c*** find roots by spline interpolation ***
     ...
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
      common/mtab/we(2),de(2),ke(2),wtry,bm
c sfrench start
c      dimension x(20),det(20),qx(3,20),wrk(60),wt(*),ichar(4)
c      data tol/1.d-9/,itmax/15/,ichar/' s',' t',' s',' c'/
      character*4 kchar
...
c sfrench start
c     my previous fix for the ichar -> kchar issue is now also removed
c     and replaced by a call to store_mode.
c     we also remove the call to modout, that the eigenfunctions are 
c     not actually written to disk.
        call store_mode( nord, l, wmhz, cvel, gcom, qmod, ifstore_eig )
        if ( ifstore_eig .eq. 1 ) then
          call store_eig_wrap
        end if
cc        write(iout,200) nord,ichar(jcom),l,cvel,wmhz,tcom,gcom,qmod,
c        write(iout,200) nord,kchar(jcom),l,cvel,wmhz,tcom,gcom,qmod,
c     +      wdiff
c  200   format(i5,a2,i5,6g16.7)
c        call modout(b,qmod,gcom,ioeig,ncall)
c sfrench stop
      else
        cg=5000./vn
...
c sfrench start
c     same here - file i/o is again commented out
cc        write(iout,200) nord,ichar(jcom),l,cvel,wmhz,tcom,gcom
...
c replaces ...
c      read(iin,100) (ititle(i),i=1,20)
c  100 format(20a4)
c      read(iin,*) ifanis,tref,ifdeck
c      if(ifdeck.eq.0) go to 1000
cc*** card deck model ***
c      read(iin,*) n,nic,noc
c      read(iin,*) (r(i),rho(i),vpv(i),vsv(i),
c     +     qkappa(i),qshear(i),vph(i),vsh(i),eta(i),i=1,n)
c  105 format(f8.0, 3f9.2, 2f9.1, 2f9.2, f9.5)
c sfrench stop
...
c*** write out model ***
c sfrench start
c     i/o once again removed ...
c        write(iout,900) (ititle(k),k=1,20),tref
c  900   format(1x,20a4,' ref per =',f6.1,' secs',///,2x,'level',
c     1   4x,'radius',8x,'rho',9x,'vpv',9x,'vph',9x,'vsv',
c     2   9x,'vsh',9x,'eta',9x,'qmu ',8x,'qkap',/)
